    ...
    def query_by_path(self, file_path=None, regex=False, re_pattern: Pattern = None):
        with self._manager_lock:
            if self.should_keep_running():
                return self._file_cache.query_by_path(file_path, regex, re_pattern)
            else:
                return list()

    def query_by_key_range(self, from_key, to_key=None):
        with self._manager_lock:
            if self.should_keep_running():
                return self._file_cache.query_by_key_range(from_key, to_key)
            else:
                return list()
